...
/**
 * The archive browser: toolbar, feed, grid, galaxy, modal player and the
 * content advisory, over whichever collection the current page shows.
 *
 * Both `/archive` and `/experience-logs` render exactly this component, and
 * nothing else — so they behave identically by construction rather than by
 * being kept in step. Everything that differs between them (which views are
 * offered, whether items have bins, what is being counted, where the data
 * comes from) is declared once, in shared/utils/collections.ts, and reaches
 * this component through `useArchive().spec`.
 *
 * Client-rendered in effect: the data loads in `onMounted`, because the
 * archive's dataset is an 18MB static file with no business being fetched
 * during SSR.
 *
    ...
    >
      ...
      >
        ...
        <p
          ...
          class="notice"
        >
          Media is not configured, so {{ archive.spec.noun.many }} will not play. Search and filtering still work.
        </p>
